...
| Workflow | 觸發條件 | 檢查內容 |
|----------|----------|----------|
| **backend-ci.yml** | push, pull_request | 單元測試 + 整合測試 + golangci-lint v2.7 |
...
**典型現象**：
- 前端按鈕點測看起來正常；
- 實際透過 API/客戶端呼叫時返回 `Service temporarily unavailable` 或提示無可用帳號；
...
- [ ] `go test -tags=unit ./...` 透過
- [ ] `go test -tags=integration ./...` 透過
